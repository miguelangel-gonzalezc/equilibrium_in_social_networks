...
stationary_edge_densities = np.zeros((len(wave_numbers)-1,len(edge_densities)))
...
for wave in tqdm(range(len(wave_numbers)-1)):
    ...
    i = (np.abs(eigenvalues - 1)).argmin()
    # We obtain the stationary abundances
    stationary_edge_abundances = abs(eigenvectors[:,i])
    densities = stationary_edge_abundances/stationary_edge_abundances.sum()
        
    stationary_edge_densities[wave] = densities
