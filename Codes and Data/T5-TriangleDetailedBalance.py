...
for k in range(N):
    ...
    for i in tqdm(range(len(order))):
        for j in range(len(order)):
            ...
            if np.sum(E)!=0: error_el = 2*np.sqrt(e.loc[order.iloc[i]['motif']]*(1-e.loc[order.iloc[i]['motif']])/np.sum(E))
            else: error_el = 0
            if np.sum(P[j])!=0: error_pr = 2*np.sqrt(p[j,i]*(1-p[j,i])/np.sum(P[j]))
            else: error_pr = 0
            if np.sum(E)!=0: error_er = 2*np.sqrt(e.loc[order.iloc[j]['motif']]*(1-e.loc[order.iloc[j]['motif']])/np.sum(E))
            else: error_er = 0
