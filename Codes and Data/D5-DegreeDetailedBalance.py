...
violations = pd.DataFrame(columns=['degree_i','degree_j','wave','n_degrees_i','n_degrees_j','n_transitions_ij','n_transitions_ji','relative_error'])
v = 0
for k in range(N):
    
    E = degree_abundances[str(k)]
    e = degree_densities[str(k)]
    ...
    for i in range(len(order)):
        for j in range(len(order)):
            ...
            if np.sum(P[i])!=0: error_pl = 2*np.sqrt(p[i,j]*(1-p[i,j])/np.sum(P[i]))
            else: error_pl = 0
            if np.sum(E)!=0: error_el = 2*np.sqrt(e.iloc[i]*(1-e.iloc[i])/np.sum(E))
            else: error_el = 0
            if np.sum(P[j])!=0: error_pr = 2*np.sqrt(p[j,i]*(1-p[j,i])/np.sum(P[j]))
            else: error_pr = 0
            if np.sum(E)!=0: error_er = 2*np.sqrt(e.iloc[j]*(1-e.iloc[j])/np.sum(E))
            else: error_er = 0
            ...
            if detailed_balance[k,i,j] == 0:
                if j>i:
                    violations.loc[v,'degree_i'] = order[i]
                    violations.loc[v,'degree_j'] = order[j]
                    violations.loc[v,'wave'] = k
                    violations.loc[v,'n_degrees_i'] = E.iloc[i]
                    violations.loc[v,'n_degrees_j'] = E.iloc[j]
                    violations.loc[v,'n_transitions_ij'] = transition_matrices[k,i,j]
                    violations.loc[v,'n_transitions_ji'] = transition_matrices[k,j,i]
                    violations.loc[v,'relative_error'] = 2*Rij[k,i,j]/(errorRij[k,i,j])
